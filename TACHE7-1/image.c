...
Image calculer_masque_image(Image I){
    Image res = creer_image(largeur_image(I),hauteur_image(I));
...
int pixels_noir_image(Image I){
	for (UINT i = 1; i <= hauteur_image(I); i++) {
		for (UINT j = 1; j <= largeur_image(I); j++) {
